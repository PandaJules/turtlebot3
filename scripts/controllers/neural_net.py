...
w = 0
weight_space = [p for p in itertools.product([0.05, 0.1, 0.15, 0.2, 0.25, 0.3], repeat=3)]
...
bad_weights_file = os.path.join(os.path.expanduser('~'), "Desktop") + "/bad_weights_hex.txt"
bad_weights = []
...
def stabilize():
    ...
    if rospy.has_param('startXY'):
        ...
        y = rospy.get_param('startXY/y')#+random.uniform(-0.5,0.5)
    else:
